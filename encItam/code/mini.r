rm(list = ls())
setwd("/home/eric/Desktop/data/elecs/MXelsCalendGovt/reelec/encItam/data")

dat <- read.csv("miniencuestaRespuestasEMM.csv", stringsAsFactor = FALSE, header = TRUE)
colnames(dat)
head(dat)
...
tmp <- dat.freq[,c("n","word","eval")]
write.csv(tmp, "group.eval.csv")
...
wordcloud(words = dat.freq$word, freq = dat.freq$freq, max.words = 25, random.order = FALSE, ordered.colors = TRUE, colors = c("red","black","darkgreen")[factor(dat.freq$eval)])
